  ...
  return (
    <section className="px-5 pt-0 pb-10 md:pb-16">
      <div className="max-w-[var(--max-width)] mx-auto">
        <h5 className="font-epilogue text-3xl text-center text-[var(--secondary-text-color)] pb-3">{upperExtraInfo}</h5>
        <h2 className="font-epilogue text-center text-4xl md:text-7xl  text-[var(--main-primary-text-color)] font-bold pb-10">
          {sectionTitle}
        </h2>
        <div className="flex flex-wrap md:flex-wrap lg:flex-nowrap justify-between gap-5">
          {cardsDefaultData.map((card) => (
            <div
              key={card.id}
              className="w-full md:w-[48.5%] lg:w-[32%] bg-[var(--main-theme-background)] rounded-2xl shadow-md p-6 cursor-pointer"
            >
              ...
              <h3 className="w-auto md:w-[300px] font-epilogue text-2xl md:text-3xl text-white font-medium border-b border-white pt-4 pb-7 md:pb-24">
                {card.title}
              </h3>
              <p className="text-white text-[16px] font-light mb-10 pt-5">
                {card.description}
              </p>
              <Link
                href={card.btnUrl}
